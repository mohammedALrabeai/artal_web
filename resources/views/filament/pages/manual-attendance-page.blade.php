        ...
        @push('scripts')
                ...
                document.addEventListener('DOMContentLoaded', () => {
                    ...
                    function createColumnDefs(monthStr, editableDateStr) {
                        ...
                        const totalValueGetter = (params) => {
                            const attendance = params.data.attendance || {};
                            const validStates = ['M', 'D', 'D8', 'X', 'A', 'M8', 'N8', 'N12', 'M12', 'N', 'COV', 'OFF',
                                'BEFORE', 'AFTER', 'PV', 'UV', 'SL', 'UL'
                            ];
                            return Object.values(attendance).filter(val => val && validStates.includes(val.status))
                                .length;
                        };
                        ...
                        const summaryColumns = [{
                            ...
                            headerName: "إجمالي\nالتغطية",
                            valueGetter: (p) => Object.values(p.data.attendance || {}).filter(v => v && v
                                .has_coverage).length,
                    ...
                    async function quickSaveAttendance(employeeId, date) {
                        ...
                        try {
                            ...
                            const result = await livewireComponent.call('quickSaveStatus', employeeId, date,
                ...
                <div class="p-4 space-y-4 bg-white border border-gray-300 rounded-lg shadow-lg" style="width: 350px;">
                    ...
                    <div data-ref="coverageSection" class="p-3 space-y-3 border border-gray-200 rounded-md">
                        <div class="flex items-center justify-between">
                           ...
                           <label class="relative inline-flex items-center cursor-pointer" data-ref="coverageLabel">
                                <input type="checkbox" data-ref="coverageToggle" class="sr-only">
                        ...
                        <div data-ref="employeeSearchContainer" class="hidden">
                            <label class="block mb-1 font-semibold text-gray-700">الإسناد البديل</label>
                            <div class="relative">
                                <input  type="text"
                                        data-ref="employeeSearchInput"
                                        placeholder="ابحث بالاسم أو الهوية أو الموقع..."
                                        class="w-full p-2 border border-gray-300 rounded-md">
                                <!-- زر البحث -->
                                <button data-ref="searchBtn"
                                        class="absolute inset-y-0 left-0 flex items-center px-3 text-primary-600">
                                    🔍
                                </button>
                                <div data-ref="searchResults"
                                    class="absolute z-10 hidden w-full mt-1 overflow-y-auto bg-white border border-gray-300 rounded-md max-h-40">
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            `;
                        ...
                        attachEventListeners() {
                            ...
                            // زر وفعل Enter
                            this.refs.searchBtn.addEventListener('click', () => this.searchAssignments());
                            this.refs.employeeSearchInput.addEventListener('keydown', (e) => {
                                if (e.key === 'Enter') {
                                    e.preventDefault();
                            ...
                            this.refs.coverageLabel.addEventListener('click', (e) => {
                                ...
                                setTimeout(() => {
                                    const checked = this.refs.coverageToggle.checked;
                                    this.handleStateChange('has_coverage', checked);
                                }, 0); // defer إلى بعد تغيير DOM
                            });
                        }

                        async searchAssignments() {
                            const term = this.refs.employeeSearchInput.value.trim();
                            if (!term) return;

                            // استعلام مباشر — دون تخزين مؤقت
                            try {
                                const resp = await fetch('/api/assignments-list?q=' + encodeURIComponent(term));
                                ...
                                if (list.length) {
                                    ...
                                    list.forEach(a => {
                                        ...
                                        row.className = 'p-2 hover:bg-primary-100 cursor-pointer';
                                        row.textContent = `${a.name} – ${a.location} (${a.shift})`;
                                        row.addEventListener('click', () => {
                                            this.handleStateChange('replaced_record_id', a.id);
                                            this.handleStateChange('replaced_employee_name', a.name);
                                            this.refs.searchResults.classList.add('hidden');
                                            this.updateUI();
                                        });
                                        this.refs.searchResults.appendChild(row);
                                    });
                                } else {
                                    this.refs.searchResults.classList.add('hidden');
                                }
                        ...
                        handleStateChange(key, value) {
                            ...
                            if (key === 'has_coverage' && !value) {
                                this.state.replaced_record_id = null;
                                this.state.replaced_employee_name = '';
                            }
                            this.updateUI();
                        }
                        ...
                        updateUI() {
                            ...
                            //  const showCoverageSection = this.state.status === 'COV';
                            //   this.refs.coverageSection.style.display = showCoverageSection ? 'block' : 'none';
                            this.refs.coverageSection.style.display = 'block';
                    ...
                    const gridOptions = {
                            ...
                            tooltipValueGetter: (p) => {
                                if (p.colDef.field && p.colDef.field.startsWith('attendance.')) {
                                    ...
                                    if (val.has_coverage) {
                                        ...
                                        if (val.replaced_employee_name) {
                                            tooltip += `\nالبديل: ${val.replaced_employee_name}`;
                                        }
                                    }
                        ...
                        onGridReady: params => {
                            ...
                            if (livewireComponent) {
                                ...
                                const summaryColIds = ['summary_x', 'summary_m', 'summary_cov', 'summary_off',
                                    'summary_d', 'summary_d8', 'summary_before', 'summary_after', 'summary_pv',
                                    'summary_uv', 'summary_sl', 'summary_ul', 'summary_total_coverage',
                                    'summary_total'
                                ];
                        ...
                        onCellValueChanged: (event) => {
                            ...
                            event.api.refreshCells({
                                rowNodes: [event.node],
                                columns: ['summary_x', 'summary_m', 'summary_cov', 'summary_off',
                                    'summary_d', 'summary_d8', 'summary_before', 'summary_after',
                                    'summary_pv', 'summary_uv', 'summary_sl', 'summary_ul',
                                    'summary_total_coverage', 'summary_total'
                                ]
                            });
                        }
                    };
                    ...
                    document.getElementById('toggleSummaryBtn').addEventListener('click', () => {
                        ...
                        const summaryColIds = ['summary_x', 'summary_m', 'summary_cov', 'summary_off', 'summary_d',
                            'summary_d8', 'summary_before', 'summary_after', 'summary_pv', 'summary_uv',
                            'summary_sl', 'summary_ul', 'summary_total_coverage', 'summary_total'
                        ];
